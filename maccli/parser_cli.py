...
def add_instance_parser(subparsers):
    ...
    create_parser.add_argument('-p', '--provider', default="manageacloud", choices=["manageacloud", "rackspaceus", "amazon", "digitalocean"],
    ...
    create_parser.add_argument('-r', '--release', default="any", choices=["any", "ubuntu", "centos", "debian", "amazon"],
                               help="Choose the distribution (default is 'any', which is the "
                                    "best match for 'configuration' parameter)")

    create_parser.add_argument('-hw', '--hardware', help="Choose the hardware settings. It only applies if parameter 'deployment' is 'production'. "
